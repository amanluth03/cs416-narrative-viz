...
The triggers in my narrative visualization connect what the user does to changes in state. Clicking the Back or Next buttons, the scene-counter dots, or pressing the Left/Right arrow keys all move the visualization to a new scene and redraw the chart. Hovering over a point in the scatter plot makes the tooltip show up, move with the cursor, and disappear once the mouse leaves. Clicking a row in the legend turns that group on or off and updates the chart to match. To communicate these options to the reader, I made sure the scene counter and Back/Next buttons are always visible, so navigation is obvious from the first scene. I also added a pill of text that only shows up in scene 3, so the reader knows exactly when they're able to hover and filter. The legend rows also use a pointer cursor and dim slightly when toggled off, which signals to the reader that they're actually clickable controls and not just a static key.
